...
class AccountTypeController {
    ...
    def csv (){
        ...
        def accounts =  getAccountTypesForEntity(params)

        if (accounts.totalCount > CsvExporter.MAX_RESULTS) {
            flash.error = message(code: 'error.export.exceeds.maximum')
            flash.args = [CsvExporter.MAX_RESULTS]
            redirect action: 'list', id: params.id

        } else {
            DownloadHelper.setResponseHeader(response, "accountTypes.csv")
            Exporter<AccountTypeDTO> exporter = CsvExporter.createExporter(AccountTypeDTO.class);
            render text: exporter.export(accounts), contentType: "text/csv"
        }
    }
