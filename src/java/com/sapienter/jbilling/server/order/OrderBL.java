            ...
            if (updatedDTO.getId() == null) {
                ...
                if (updatedDTO.getParentOrder() != null) {
                    ...
                    if (parentOrder != null) {
                        ...
                        parentOrder.addChildOrder(updatedDTO);
                    }
                }
            } else {
                ...
                if (updatedDTO.getParentOrder() == null && targetOrder.getParentOrder() != null
                    ...
                    targetOrder.getParentOrder().removeChildOrder(targetOrder);
                    ...
                    if (parentOrder != null) {
                        ...
                        parentOrder.addChildOrder(targetOrder);
                    }
                }
    	...
    	if (orderIds != null && !orderIds.isEmpty()) {
    		...
    		for (OrderDTO orderDTO : orderArray) {
    			if (orderDTO.getId().intValue() != primaryOrder.getId().intValue()) {
	    			...
	    			primaryOrder.addChildOrder(orderDTO);
	...
	*/
    ...
    public void updatePeriods(Integer languageId, OrderPeriodDTO periods[]) {
        for (OrderPeriodDTO period : periods) {
            ...
            period.getCompany().addOrderPeriod(period);
        }
    }
        ...
        if (dto == null) {
            ...
            if (ws.getChildLines() != null) {
                for (int i = 0; i < ws.getChildLines().length; i++) {
                    ...
                    dto.addChildLine(getOrderLine(childLine, linesMapForHierarchy));
                }
            }
        }
        return dto;
...
//          orderLineDAS.delete(line);
    ...
    public void addAssetToOrderLine(
	    ...
	    //TODO replace below with: assetDTO.addAssignment(assignment);
	    assetDTO.addAssignment(assignment);

	    line.addAssetAssignment(assignment);
        ...
        if (retValue == null) {
            ...
            if (order.hasDiscountLines()) {
    	        for (DiscountLineWS discountLine : order.getDiscountLines()) {
    	            if (discountLine != null) {
    	                ...
    	                retValue.addDiscountLine(discountLineDto);
    	            }
    	        }
            }
            ...
            if (order.getChildOrders() != null) {
                for (OrderWS childOrder : order.getChildOrders()) {
                    retValue.addChildOrder(getDTO(childOrder, ordersHierarchyMap, orderLinesHierarchyMap));
                }
            }
