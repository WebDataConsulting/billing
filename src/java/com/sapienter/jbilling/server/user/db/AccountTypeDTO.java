...
public class AccountTypeDTO extends AbstractDescription implements java.io.Serializable, Exportable {
    ...
    public void addAccountInformationType(AccountInformationTypeDTO AccountInformationType ){
	    this.informationTypes.add(AccountInformationType);
    }

    public void removeAccountInformationType(AccountInformationTypeDTO AccountInformationType){
	    this.informationTypes.remove(AccountInformationType);
    }
    ...
    public void addPaymentMethodType(PaymentMethodTypeDTO paymentMethodType){
	    this.paymentMethodTypes.add(paymentMethodType);
    }

    public void removePaymentMethodType(PaymentMethodTypeDTO paymentMethodType){
    ...
    public Object[][] getFieldValues() {
        ...
        StringBuilder informationTypes = new StringBuilder();
        for (AccountInformationTypeDTO type : this.informationTypes) {
            informationTypes.append(type.getName()).append(',');
        }

        StringBuilder paymentMethodTypes = new StringBuilder();
        for ( PaymentMethodTypeDTO paymentMethod : this.paymentMethodTypes) {
            paymentMethodTypes.append(paymentMethod.getMethodName()).append(',');
        }
        ...
        values.add(
                new Object[] {
                        ...
                        creditLimit,
                        invoiceDesign,
                        billingCycle.getNextInvoiceDayOfPeriod()+billingCycle.getSubscriptionPeriod().getDescription() ,
                        preferredNotificationAitId,
                        creditNotificationLimit1,
                        creditNotificationLimit2,
                        (invoiceDeliveryMethod != null ? invoiceDeliveryMethod.getId() : null),
                        (language != null ? language.getDescription() : null),
                        (currency != null ? currency.getDescription() : null),
                        paymentMethodTypes.toString(),
                        informationTypes.toString(),
                }
        );
